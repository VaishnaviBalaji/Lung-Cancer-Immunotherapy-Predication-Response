...
def save_feedback(patient_id, predicted_response, actual_response, clinician_comments):
    conn = sqlite3.connect('feedback.db')
    c = conn.cursor()
    c.execute('''INSERT INTO feedback (patient_id, predicted_response, actual_response, clinician_comments, timestamp)
                 VALUES (?, ?, ?, ?, ?)''', 
                 (patient_id, predicted_response, actual_response, clinician_comments, datetime.now()))
    conn.commit()
    conn.close()
    print(f"Feedback saved: {patient_id}, {predicted_response}, {actual_response}, {clinician_comments}")  # Debug print
...
if st.button('Predict'):
    ...
    with st.form(key='feedback_form'):
       ...
       actual_response = st.selectbox('Actual Response', ['Positive', 'Negative'])
       clinician_comments = st.text_area('Additional Comments')
       submit_button = st.form_submit_button(label='Submit Feedback')
    
       if submit_button:
           predicted_response = 'Positive' if prediction[0] == 1 else 'Negative'
           save_feedback(patient_id, predicted_response, actual_response, clinician_comments)
           st.session_state.feedback_submitted = True

    if st.session_state.feedback_submitted:
       st.success('Feedback submitted successfully!')
       st.session_state.feedback_submitted = False  # Reset for next submission

# Option to view past feedback
if st.sidebar.checkbox('View Past Feedback'):
...
# Add a button to refresh the feedback display
if st.button('Refresh Feedback Display'):
    st.experimental_rerun()
# ...
# After displaying the prediction results, add this code for feedback
# Feedback form
# After displaying the prediction results, add this code for feedback
